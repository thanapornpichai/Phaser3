...
export function sendPrivateChat(remote: IPacket | undefined, senderId: number, message: string) {
    if (remote === undefined) return;
    remote.send(JSON.stringify({ name: "PrivateChat", data: { senderId, message } }));
}
